...
export async function GET() {
  ...
  // Only return conversations for the authenticated user — userId is from Clerk session, not user input
...
export async function POST(req: Request) {
  ...
  // Sanitize title — strip HTML tags, limit length
  let title = "New conversation";
  if (typeof body.title === "string" && body.title.trim()) {
    title = escapeHtml(body.title.trim().slice(0, 200));
  }

  // userId comes from Clerk session — no injection possible
...
export async function DELETE(req: Request) {
  ...
  // Validate UUID format to prevent malformed input
  if (!isValidUuid(id)) {
    return NextResponse.json({ error: "Invalid conversation id" }, { status: 400 });
  }

  // Delete only if conversation belongs to the authenticated user
  await db
